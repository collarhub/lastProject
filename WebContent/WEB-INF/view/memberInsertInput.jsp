	...
	<body>
		...
		<div id="center">
			<form action="insertInput.do" method="post" style = "color:black">
				...
				<label>아이디</label> <input type="text" name="member_id" onkeydown="inputIdChk()" value="${member.member_id }" autofocus required>
				&nbsp; <a href="idCheck.do">아이디 체크하러 가기</a> <br>
				<label>비밀번호</label> <input type="password" name="password" value="${member.password_id }" required><br>
				<label>닉네임</label><input type="text" name="nickname"  value="${member.nickname }"required><br>
				<label>이메일</label><input type="text" name="email" placeholder="xxxxxx@gmail.com" value="${member.email }" required><br>
				<label>생년월일</label><input type="date" name="birth_date"  value="${member.birth_date }"required><br>
				<label>전화번호</label><input type="text" name="phone"  value="${member.phone }"required><br>
				<label>주소</label><input type="text" name="addr"  value="${member.addr }"required><br>
				<label>사진</label><input type="file" name="photo"  onchange="readURL(this);" ><br>
				<img id="UploadedImg" src="default.jpg" width="85" height="111"><br><br>
				<label>점수</label><input type="text" name="rank"  value="0" disabled ><br><br>
				
				<input type="submit" value="회원가입">
				<input type="reset"  value="재입력">   ${error1 }
			</form>
		</div>		
